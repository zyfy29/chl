...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		colUsernameIdx, colPasswordIdx, colTokenIdx := -1, -1, -1
		for i, cell := range res.ValueRange.Data[0] {
			if cell == config.Conf.Header.Username {
				colUsernameIdx = i
			} else if cell == config.Conf.Header.Password {
				colPasswordIdx = i
			} else if cell == config.Conf.Header.Token {
				colTokenIdx = i
			}
		}
		if colUsernameIdx == -1 || colPasswordIdx == -1 || colTokenIdx == -1 {
			cmd.Println("Required columns (Username, Password, Token) not found in the header row.")
			return
		}

		conn, err := grpc.NewClient(config.Conf.Base.GrpcAddr, grpc.WithInsecure())
		if err != nil {
			cmd.PrintErrf("Error connecting to gRPC server: %v\n", err)
			return
		}
		defer conn.Close()
		client := login.NewTokenServiceClient(conn)
		...
		for i, row := range res.ValueRange.Data {
			...
			if row[colTokenIdx] != "" {
				cmd.Printf("Row %d: Token already exists. Skipping login.\n", i)
				continue
			}
			...
			resp, err := client.GetPocketToken(cmd.Context(), &login.LoginRequest{
				Username: username,
				Password: password,
			})
			if err != nil {
			...
			if err := feishu.Api.WriteCellData(config.Conf.Table.TableToken, config.Conf.Table.SheetID, feishu.Index2Range(i, colTokenIdx), resp.Token); err != nil {
				cmd.Printf("Row %d: Error writing token '%s' to cell [%d, %d]: %v\n", i, resp.Token, i, colTokenIdx, err)
			} else {
